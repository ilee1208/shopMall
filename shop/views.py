...
#model불러올때
from shop.models import Item, Basket, Order, Comment

#form 불러올때
from shop.forms import ItemForm,  BasketForm, OrderForm, CommentForm    #PostSearchForm
...
#from shop.decorators import user_is_shop_author
...
def itemList(request):
    ...
    return render(request, 'shop/itemList.html', data)
...
def itemAdd(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            messages.success(request, 'Shop was successfully added!')
    ...
    return render(request, 'shop/itemForm.html', data)
...
def itemRemove(request, item_id):
    shop = get_object_or_404(Item, pk=item_id)
    shop.delete()
    messages.success(request, 'Shop-item was successfully removed!')
    return redirect('shop:itemList')
...
def itemSearch(request):
    ...
    template_name = 'shop/itemList.html'
...
def orderForm(request):
    ...
    if request.method == 'POST':
        ...
        return render(request, 'shop/orderForm.html', data)
...
def orderList(request):
    ...
    return render(request, 'shop/orderList.html', data)
...
def basketInsert(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('shop:basketList')
    else:
        data = {
            'form': BasketForm(),
            'basket_id': 0,
        }
    return render(request, 'shop/basketList.html', data)
...
def commentAdd(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return HttpResponseRedirect(reverse('shop:itemDetail', args=(item_id,)))
            # return render(request, 'shop:itemDetail', request.POST['item_id'])
